...
class BankAccount:
    ...
    def create_account():
        ...
        print("1. Current Account (No interest, unlimited withdrawals)")
        print("2. Savings Account (Monthly interest, limited withdrawals)")
        ...
        if account_type_choice == "2":
            ...
            print("- 3% monthly interest on your balance")
            print("- Limited to 3 withdrawals per month")
            print("- Minimum balance requirement: 100,000")
    ...
    def withdraw():
        ...
        # Check sender exists and get balance and account type
        BankAccount.cursor.execute(
        ...
        sender_account_type = sender_result[1]

        # Check for savings account restrictions
        if sender_account_type == "savings":
            # Check minimum balance requirement (100,000)
            if sender_balance - amount < 100000:
                print("Transaction denied: Savings accounts must maintain a minimum balance of 100,000")
                return
                
            # Check withdrawal count in current month
            import datetime
            ...
            BankAccount.cursor.execute(
                ...
                """, 
                (sender_card, current_month)
            )
            
            withdrawal_count = BankAccount.cursor.fetchone()[0]
            
            if withdrawal_count >= 3:
                print("Transaction denied: Savings accounts are limited to 3 withdrawals per month")
                return
        ...
        BankAccount.cursor.execute(
            """
            INSERT INTO transactions (sender_card, receiver_card, amount, transaction_type, transaction_date)
            VALUES (%s, %s, %s, %s, NOW())
            """,
            (sender_card, receiver_card, amount, "withdrawal")
        )
