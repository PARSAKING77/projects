...
class BankAccount:
    ...
    def option():
        ...
        print("4. Apply Monthly Interest📈 (for Saving Accounts)")
        print("5. Check Account Type🔎")
    ...
    @staticmethod
    def apply_interest():
        """Add 2% monthly interest to all saving accounts"""
        BankAccount.cursor.execute(
            "SELECT credit_number, balance FROM accounts WHERE account_type = 'saving'"
        )
        saving_accounts = BankAccount.cursor.fetchall()
        ...
        for acc in saving_accounts:
            credit_number, balance = acc
            new_balance = float(balance) * 1.02  # Apply 2% interest
            BankAccount.cursor.execute(
                "UPDATE accounts SET balance = %s WHERE credit_number = %s",
                (new_balance, credit_number)
            )

        BankAccount.db.commit()
        print("Monthly interest applied successfully to all Saving Accounts📈")
